...
var (
	...
	createTableSQL = `CREATE TABLE IF NOT EXISTS %s.%s (%s create_time DateTime DEFAULT NOW()) ENGINE=ReplacingMergeTree(%s) PARTITION BY toYYYYMMDD(create_time) ORDER BY (%s) SETTINGS index_granularity = 8192;`
