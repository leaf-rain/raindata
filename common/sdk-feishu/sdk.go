...
func (sdk *SDKFeishu) getSecret() string {
	return base64.RawURLEncoding.EncodeToString([]byte(sdk.cfg.ClientSecret))
}

func (sdk *SDKFeishu) GetLoginUrl(state string) string {
	if state == "" {
		state = fmt.Sprintf("%d", rand.Int())
	}
	url := sdk.cfg.AuthCodeURL(state, oauth2.S256ChallengeOption(sdk.getSecret()))
	return url
}

func (sdk *SDKFeishu) GetUserAccessToken(ctx context.Context, code string) (*oauth2.Token, error) {
	return sdk.cfg.Exchange(ctx, code, oauth2.VerifierOption(sdk.getSecret()))
}
