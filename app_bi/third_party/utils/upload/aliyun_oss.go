...
func (*AliyunOSS) UploadFile(file *multipart.FileHeader) (string, string, error) {
	...
	yunFileTmpPath := svc.conf.AliyunOSS.BasePath + "/" + "uploads" + "/" + time.Now().Format("2006-01-02") + "/" + file.Filename
	...
	return svc.conf.AliyunOSS.BucketUrl + "/" + yunFileTmpPath, yunFileTmpPath, nil
}
...
func NewBucket() (*oss.Bucket, error) {
	// 创建OSSClient实例。
	client, err := oss.New(svc.conf.AliyunOSS.Endpoint, svc.conf.AliyunOSS.AccessKeyId, svc.conf.AliyunOSS.AccessKeySecret)
	if err != nil {
		return nil, err
	}

	// 获取存储空间。
	bucket, err := client.Bucket(svc.conf.AliyunOSS.BucketName)
	if err != nil {
		return nil, err
	}
