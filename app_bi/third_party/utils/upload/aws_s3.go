...
func (*AwsS3) UploadFile(file *multipart.FileHeader) (string, string, error) {
	...
	filename := svc.conf.AwsS3.PathPrefix + "/" + fileKey
	...
	_, err := uploader.Upload(&s3manager.UploadInput{
		Bucket: aws.String(svc.conf.AwsS3.Bucket),
	...
	return svc.conf.AwsS3.BaseURL + "/" + filename, fileKey, nil
}
...
func (*AwsS3) DeleteFile(key string) error {
	session := newSession()
	svc := s3.New(session)
	filename := svc.conf.AwsS3.PathPrefix + "/" + key
	bucket := svc.conf.AwsS3.Bucket
...
func newSession() *session.Session {
	sess, _ := session.NewSession(&aws.Config{
		Region:           aws.String(svc.conf.AwsS3.Region),
		Endpoint:         aws.String(svc.conf.AwsS3.Endpoint), //minio在这里设置地址,可以兼容
		S3ForcePathStyle: aws.Bool(svc.conf.AwsS3.S3ForcePathStyle),
		DisableSSL:       aws.Bool(svc.conf.AwsS3.DisableSSL),
		Credentials: credentials.NewStaticCredentials(
			svc.conf.AwsS3.SecretID,
			svc.conf.AwsS3.SecretKey,
			"",
		),
	})
	return sess
