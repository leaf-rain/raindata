...
func (s *SysAutoCodeHistory) BeforeCreate(db *gorm.DB, pathRoot, werRoot, codeServer string) error {
	templates := make(map[string]string, len(s.Templates))
	for key, value := range s.Templates {
		server := filepath.Join(pathRoot, codeServer)
		{
		...
		web := filepath.Join(pathRoot, werRoot)
