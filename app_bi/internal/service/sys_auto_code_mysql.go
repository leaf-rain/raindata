...
// GetDB 获取数据库的所有数据库名
func (svc *autoCodeMysql) GetDB(businessDB string) (data []dto.Db, err error) {
	var entities []dto.Db
...
func (svc *autoCodeMysql) GetTables(businessDB string, dbName string) (data []dto.Table, err error) {
	var entities []dto.Table
	sql := `select table_name as table_name from information_schema.tables where table_schema = ?`
	err = svc.data.SqlClient.Raw(sql, dbName).Scan(&entities).Error
	return entities, err
}

// GetColumn 获取指定数据库和指定数据表的所有字段名,类型值等
func (svc *autoCodeMysql) GetColumn(businessDB string, tableName string, dbName string) (data []dto.Column, err error) {
...
WHERE 
    c.TABLE_NAME = ? 
    AND c.TABLE_SCHEMA = ?;`
	err = svc.data.SqlClient.Raw(sql, tableName, dbName).Scan(&entities).Error
	return entities, err
}
